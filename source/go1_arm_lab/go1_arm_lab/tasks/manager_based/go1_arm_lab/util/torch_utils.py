...
def quat_mul(a, b):
    ...
    Args:
        a: First quaternion in [x, y, z, w] format
        b: Second quaternion in [x, y, z, w] format
        
    Returns:
        Result quaternion in [x, y, z, w] format
    """
...
@torch.jit.script   
def quat_rotate_inverse_wxyz(quat_wxyz: torch.Tensor, vec: torch.Tensor) -> torch.Tensor:
    """
    Applies the inverse of a quaternion rotation to a 3D vector.
    Assumes quaternion input is in [w, x, y, z] order.
    """
    quat_xyzw = torch.cat([quat_wxyz[:, 1:], quat_wxyz[:, :1]], dim=-1)
    return quat_rotate_inverse(quat_xyzw, vec)
