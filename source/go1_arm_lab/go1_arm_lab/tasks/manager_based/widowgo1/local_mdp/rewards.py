...
# -- Walk these ways
def reward_tracking_contacts_shaped_force(env: ManagerBasedRLEnv, sensor_cfg: SceneEntityCfg, gait_force_sigma: float) -> torch.Tensor:
    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
    net_contact_forces = contact_sensor.data.net_forces_w_history
    foot_forces = torch.max(torch.norm(net_contact_forces[:, :, sensor_cfg.body_ids], dim=-1), dim=1)[0]

    # 步态目标生成逻辑（完全自包含）
    base_vel = env.command_manager.get_command("base_velocity")
    walking_mask = (
        (torch.abs(base_vel[:, 0]) > 0.2) |
        (torch.abs(base_vel[:, 1]) > 0.2) |
        (torch.abs(base_vel[:, 2]) > 0.5)
    )

    # 步态参数
    frequencies, phases, offsets, bounds, durations = 2, 0.5, 0, 0, 0.5

    # 关键：每次reward都用当前step的gait_indices（不依赖env任何变量）
    # 这里假设每个环境的gait_indices都从0开始，步进一次
    gait_indices = torch.zeros(env.num_envs, dtype=torch.float, device=env.device)
    gait_indices = torch.remainder(gait_indices + env.step_dt * frequencies, 1.0)
    gait_indices[~walking_mask] = 0
    ...
    def smoothing_multiplier(idx):
        return (
            smoothing_cdf_start(torch.remainder(idx, 1.0)) * (1 - smoothing_cdf_start(torch.remainder(idx, 1.0) - 0.5))
            + smoothing_cdf_start(torch.remainder(idx, 1.0) - 1) * (1 - smoothing_cdf_start(torch.remainder(idx, 1.0) - 0.5 - 1))
        )
    desired_contact = torch.stack([smoothing_multiplier(idx) for idx in foot_indices], dim=1)
...
def reward_tracking_contacts_shaped_vel(env: ManagerBasedRLEnv, sensor_cfg: SceneEntityCfg, gait_vel_sigma: float) -> torch.Tensor:
    asset = env.scene["robot"]
    foot_velocities = torch.norm(asset.data.body_lin_vel_w[:, sensor_cfg.body_ids, :], dim=2)  # shape: (num_envs, 4)
    ...
    # 每次reward都用当前step的gait_indices（不依赖env任何变量）
    gait_indices = torch.zeros(env.num_envs, dtype=torch.float, device=env.device)
    gait_indices = torch.remainder(gait_indices + env.step_dt * frequencies, 1.0)
    gait_indices[~walking_mask] = 0
...
def reward_feet_height(env: ManagerBasedRLEnv, sensor_cfg: SceneEntityCfg) -> torch.Tensor:
    # 计算脚部高度（从场景中获取脚部body的位置）
    asset = env.scene["robot"]  # 假设机器人资产名为"robot"
    feet_positions = asset.data.body_state_w[:, sensor_cfg.body_ids, :3]  # shape: (num_envs, 4, 3)
    feet_height = feet_positions[:, :2, 2]  # 只取前脚的高度 (z坐标)
